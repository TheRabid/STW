...
function start(response, request) {
  ...
  var cookies = parseCookies(request);
	if(cookies['userid'] == null){
		var body = utils.generateResponse("login");
    ...
    database.checkLogin(cookies['userid'],function(err,success){
      if(!success){
        var body = utils.generateResponse("loginFailed");
    		response.writeHead(200, {
    			"Content-Type" : "text/html"
    		});
    		response.write(body);
        ...
        var body = utils.generateResponse("logged");
    		response.writeHead(200, {
    			"Content-Type" : "text/html"
    		});
    		response.write(body);
      		...
      		database.blobMemo(memoId, function(err, blob, name){
      			if(blob !== null && name !== null){
      				...
      				for (var i = 0; i < blob.length; i += 2) {
      					 a.push("0x" + blob.substr(i, 2));
      				}
      				console.log(a);
      				var buffered = new Buffer(a,"hex");
 ...
 * Muestra el html de inicio por pantalla
 */
function doLogin(response, request) {
	console.log("Request handler 'doLogin' was called.");

	// Procesamiento del formulario
	var form = new formidable.IncomingForm();
	console.log("-About to parse user");
	form.parse(request, function(error, fields, files) {
		// Si no hay errores
		if(!error){
			...
			// Intentar login
			database.loginUser(user,pass,function(err,success,id){
				if(err){
					var body = utils.generateResponse("loginFailed");
					response.writeHead(200, {
						"Content-Type" : "text/html"
 ...
 * Muestra el html de inicio por pantalla
 */
function doLogout(response, request) {
	console.log("Request handler 'doLogout' was called.");
	var body = utils.generateResponse("login");
	response.writeHead(200, {
