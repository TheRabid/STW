...
var loginBodyFailed = '<h2>' +
'Login' +
'</h2>' + '<h3> Login failed. Incorrect password </h3>' +
'<form action="/login" enctype="multipart/form-data" method="post">' +
'User:<br>' +
'<input type="text" name="user" required><br>' +
'Password:<br>' +
...
function generateResponse(kind) {
  ...
  if(kind === "logged"){
    ...
    returned = '<html>' + head + '<body>' + mainHeader + loginBody +
    '</body></html>';
  } else if (kind === "loginFailed") {
    returned = '<html>' + head + '<body>' + mainHeader + loginBodyFailed +
    '</body></html>';
    ...
    returned = buttonAndEnd + logoutButton;
    ...
    returned = buttonList + buttonAndEnd + logoutButton;
